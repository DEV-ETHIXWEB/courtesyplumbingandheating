...
export const POST: APIRoute = async ({ request, site }) => {
  ...
  const html = `
    ...
    <table cellpadding="6" style="border-collapse:collapse">
      ${rows
        ...
        .map(
          ([label, value]) =>
            `<tr><td style="border:1px solid #ddd"><strong>${escapeHtml(label)}</strong></td><td style="border:1px solid #ddd; white-space:pre-line">${escapeHtml(value)}</td></tr>`
        )
        .join('')}
    </table>
  `;
  ...
  if (!mail) {
    ...
    console.error('[chatbot-lead] RESEND_API_KEY or LEAD_FROM_EMAIL not set; lead NOT delivered.', {
      intent: lead.intent,
      zip: lead.zip,
    });
